$(function() {
	...
	playVideo[publicIndex].volume = 0.4; //初始化音量
	...
	playVideo.on('loadedmetadata', function() {
		duration.text(formatSeconds(playVideo[publicIndex].duration));
	});

	playVideo.on('timeupdate', function() {
		currentTime.text(formatSeconds(playVideo[publicIndex].currentTime));
		progress.css('width', 100 *playVideo[publicIndex].currentTime / playVideo[publicIndex].duration + '%');
	});
	...
	$('.fullScreen').on('click', function() {
		if ($(this).hasClass('cancleScreen')) {
			...
			if (playVideo[publicIndex].requestFullscreen) {
				playVideo[publicIndex].requestFullscreen();
			} else if (playVideo[publicIndex].mozRequestFullScreen) {
				playVideo[publicIndex].mozRequestFullScreen();
			} else if (playVideo[publicIndex].webkitRequestFullscreen) {
				playVideo[publicIndex].webkitRequestFullscreen();
			} else if (playVideo[publicIndex].msRequestFullscreen) {
				playVideo[publicIndex].msRequestFullscreen();
			}
	...
	var updatebar = function(x) {
		var maxduration = playVideo[publicIndex].duration; //Video 
		...
		playVideo[publicIndex].currentTime = maxduration * percentage / 100;
	};
	//音量控制
	function volumeControl(e) {
		...
		playVideo[publicIndex].volume = percentage / 100;
		e.stopPropagation();
		e.preventDefault();
	}
	...
	function playControl() {
			playPause.toggleClass('playIcon');
			if (playVideo[publicIndex].paused) {
				playVideo[publicIndex].play();
				$('.playTip').removeClass('glyphicon-play').addClass('glyphicon-pause').fadeOut();
			} else {
				playVideo[publicIndex].pause();
				$('.playTip').removeClass('glyphicon-pause').addClass('glyphicon-play').fadeIn();
			}
		}
		//关灯

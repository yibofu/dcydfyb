...
<section class="dd_borrow_box" style="margin-bottom: 90px;">
    ...
    <!--还款信息-->
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["red"] == 1): ?><section class="yhk">
        <div class="yhk_e yhk_red">
            <span><?php echo ($vo["should_money"]); ?></span>
        </div>
        <div class="yhk_e1 yhk_red">
                <li>第<?php echo ($vo["installments"]); ?>期：<i>等待还款</i></li>
                <li>到期日：<em><?php echo ($vo["repay_time"]); ?></em></li>
                <li>协议&nbsp;&nbsp;<strong><?php echo ($vo["deal_num"]); ?></strong></li>
        </div>
        <div class="yhk_detail">
            <a href="<?php echo U('Repay/detail',array('id'=>$vo.id));?>" class="yhk_redd">详情<span class="fa fa-angle-right" style="vertical-align: middle; padding-left: 2px;margin-top: -1px; font-size:16px;"></span></a>
        </div>
    </section>
	<?php else: ?>
    <section class="yhk">
        ...
        <div class="yhk_detail">
            <a href="<?php echo U('Repay/detail',array('id'=>$vo.id));?>" class="">详情<span class="fa fa-angle-right" style="vertical-align: middle; padding-left: 2px;margin-top: -1px; font-size:16px;"></span></a>
        </div>

...
<table id="dg" class="easyui-datagrid" toolbar="#toolbar" style="width:90%px;height:80%px"
       data-options="
				url: '<?php echo U('Content/bwbdlist');?>',
				rownumbers: true,
				fit:true,
...
<script type="text/javascript">
    ...
    function add(){
        ...
        url = "<?php echo U('Content/bwbdadd');?>";
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                if (row){
                    $('#fmup').form('clear');
                    $('#dlgup').dialog('open');
                    url = "<?php echo U('Content/bwbdsave');?>&id="+id;
                    $('#fmup').form('load',row);
                }else{
                    $.messager.show({
                        title: '出错啦！！',

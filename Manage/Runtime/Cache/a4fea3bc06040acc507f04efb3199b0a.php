...
<body>
	<table id="dg" class="easyui-datagrid" toolbar="#toolbar" style="width:90%px;height:80%px"
			data-options="
				url: '<?php echo U('User/datalist',array('sex'=>2));?>',
				rownumbers: true,
				fit:true,
		...
		<thead>
			<tr>
				...
				<th field="balance" align="center">账户余额</th>
				<th field="extract" align="center">提现余额</th>
				<th field="receiving" align="center">收礼金额</th>
				<th field="status" align="center" formatter="show_status">状态</th>
				<th field="ctime" align="center" width='60'>注册时间</th>
				<th field="ltime" align="center" width='60'>最后登录时间</th>
	...
	<div id="detail_dlg" class="easyui-dialog" title="个人资料修改" closed="true" buttons="#dlg-detail" style="width:420px;height:520px;">
			<div class="ftitle">个人资料修改</div>
			   <form id="detail" method="post">
				...
				<div class="fitem">
					<label>银行名称:</label>
					<input name="bank"  class="easyui-validatebox" />
				</div>
				<div class="fitem">
					<label>银行卡号:</label>
					<input name="bank_no"  class="easyui-validatebox">
				</div>
				<div class="fitem">
					<label>持卡人姓名:</label>
					<input name="bank_name"  class="easyui-validatebox">
				</div>
			</form>
			<div id="dlg-detail">
	...
	<!--个人资料结束-->
	<!--收礼记录-->
	<div id="receive-dlgs" class="easyui-window" data-options="minimizable:false,tools:'#tt'"  title="收礼记录" closed="true" style="width:1000px;height:500px;">
		   <table id="receive" class="easyui-datagrid">
			</table>
	</div>
	<!--收礼记录-->
	<!--提现记录-->
	<div id="record-dlgs" class="easyui-window" data-options="minimizable:false,tools:'#tt'"  title="提现记录" closed="true" style="width:1000px;height:600px;">
		   <table id="record" class="easyui-datagrid" style="height:500px;">
			</table>
	</div>
	<!--提现记录-->
</body>
<script type="text/javascript">
	...
	function show_manage(val,row){
		var str = '';
		str = "<a href='javascript:;' onclick='show_edit("+row.id+")' >修改</a>&nbsp;/&nbsp;<a href='javascript:;' onclick='show_detail("+row.id+")'>个人信息</a>&nbsp;/&nbsp;<a href='javascript:;' onclick='receive("+row.id+")' >收礼记录</a>&nbsp;/&nbsp;<a href='javascript:;' onclick='record("+row.id+")'>提现记录</a>";
		return str;
	}
	//datagrid状态回调
	...
	//收礼记录展示
	function receive(id){
		       $('#receive-dlgs').dialog('open');
			   $('#receive').datagrid({  
						url:'/Manerger/index.php/User/receive?uid='+id,
					columns:[[  
						{field:'receive_id',title:'用户id',width:80},  
						{field:'receive_name',title:'昵称',width:120},  
						{field:'surplus',title:'账户余额',width:120},  
	...
	//提现记录展示
	function record(id){
		       $('#record-dlgs').dialog('open');
			   $('#record').datagrid({  
					url:'/Manerger/index.php/User/record?uid='+id,
					columns:[[  
						{field:'uid',title:'用户id',width:40},  
						{field:'name',title:'昵称',width:70},  
						{field:'ctime',title:'提现时间',width:120},
						{field:'extract',title:'提现金额',width:80}, 
						{field:'bank',title:'提现银行',width:80}, 
						{field:'bank_no',title:'银行卡号',width:120}, 

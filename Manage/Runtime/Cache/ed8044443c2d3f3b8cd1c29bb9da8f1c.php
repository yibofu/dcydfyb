            ...
            <tr>
                ...
                <td>父类名称:
                    <select data-options="panelHeight:'auto',editable:false" class="easyui-combobox" name="fname" id="fname">
                        ...
                        <?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["kind"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </td>
                <td>子类名称:
                    <select data-options="panelHeight:'auto',editable:false" class="easyui-combobox" name="zname" id="zname">
                        <option value="">--请选择--</option>
                        <?php if(is_array($resul)): $i = 0; $__LIST__ = $resul;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo1["id"]); ?>"><?php echo ($vo1["zname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </td>
                <td>
...
//        alert(fname);
    ...
    function add(){
        ...
        url = "<?php echo U('Up/viewclassadd');?>";
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                if (row){
                    $('#fmup').form('clear');
                    $('#dlgup').dialog('open');
                    url = "<?php echo U('Up/viewclasssave');?>&id="+id;
                    $('#fmup').form('load',row);
                }else{
                    $.messager.show({
                        title: '出错啦！！',

    ...
    <section class="sh_box">
        ...
        <div class="detail">
            <ul>
                ...
                <li>放贷接受账号：<span><?php echo ($userres["bank_no"]); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></li>
				<li>银行<span><?php echo ($userres["bank"]); ?></span></li>
				<li>地址：<span><?php echo ($userres["bank_local"]); ?></span></li>
            </ul>
	 ...
	 <h4>审核员：<?php echo ($loanres["realname"]); ?>   <span>预留现金余额：<?php echo ($userres["account"]); ?>元&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onClick="view_account();">查看账户</a></span></h4>
            <table border="1" cellspacing="0" class="hk_detail" >
                <tr>
                	...
                	<tr>
	                    <td>第<?php echo ($repres["installments"]); ?>期</td>
	                    <td><?php echo ($repres["should_money"]); ?></td>
	                    <td><?php echo (date("Y-m-d",$repres["repay_time"])); ?></td>
	                    <td><?php echo (date("Y-m-d",$repres["real_repay_time"])); ?></td>
	                    <td><?php echo ($repres["real_money"]); ?></td>
	                    <td><?php echo ($repres["over_money"]); ?></td>
						<?php if(($repres["status"]) == "0"): ?><td><span style="color:red">未审核</span></td><?php endif; ?>
						<?php if(($repres["status"]) == "1"): ?><td><span style="color:blue">未打款</span></td><?php endif; ?>		
						<?php if(($repres["status"]) == "2"): ?><td><span style="color:green">已打款</span></td><?php endif; ?>
						
	                    <td>
							<?php if(($repres["status"]) != "2"): ?><a href="javascript:void(0);" onClick="shenhe_shoukuan(<?php echo ($repres["installments"]); ?>);">审核</a>
							<?php else: ?>
							无<?php endif; ?>
	                    </td>
	                </tr>
    ...
    <section class="sh_box">
        ...
        <div class="pic_detail">
					...
					<div style="float:left;">
						<!--<span id="big"><img style="width:120px;" src="<?php echo ($userres["card_front"]); ?>"></span>-->
							<ul style="margin-top:0px">
								<li onclick="showimg('<?php echo ($userres["card_front"]); ?>')"><img style="width:150px;cursor:pointer" src="<?php echo ($userres["card_front"]); ?>"></li>
								<li onclick="showimg('<?php echo ($userres["card_back"]); ?>')"><img style="width:150px;cursor:pointer" src="<?php echo ($userres["card_back"]); ?>"></li>
								<li onclick="showimg('<?php echo ($userres["card_hand"]); ?>')"><img style="width:150px;cursor:pointer" src="<?php echo ($userres["card_hand"]); ?>"></li>
							</ul>
					</div>
            ...
            <ul>
                ...
                <li>审核状态：
                	<span>
                		<?php if($userres["card_status"] == '1' ): ?>已通过
						<?php else: ?>
							未通过<?php endif; ?>
                	</span>
                </li>
        ...
        <div style="width:1200px;margin:0 auto;">
			<div style="width:540px;float:left;">
             <table id="datalist2" class="easyui-datagrid" title="" style="width:500px;height:300px;"
				data-options="
					url: '<?php echo U('Address/addresslist',array('id'=>$userres['id']));?>',
					rownumbers: true,
					singleSelect:true,
					fitColumns:true,
					pagination: true,
            ...
            <div style="float:left;width:600px;">
               <iframe src="__APP__/Address/index/id/<?php echo ($address_id); ?>" style="width:540px;height:300px;" name ="iframe_a"></iframe>
            </div>
            ...
            <div class="detail">
                <table border="1" cellspacing="0" class=hk_detail >
                    ...
                    <tr>
                        ...
                        <td><?php echo ($userres["job_city"]); ?></td>
                        <td><?php echo ($userres["com_phone"]); ?></td>
                        <td><?php echo (date("Y-m-d H:i:s",$userres["create_time"])); ?></td>
                        <td>
                        	<?php if($userres["job_status"] == '1' ): ?><span style="color:green;">审核成功</span>
                        	<?php elseif($userres["job_status"] == '2' ): ?>
                        		<span style="color:red;">未审核</span>
                        	<?php elseif($userres["job_status"] == '3' ): ?>
                        		<span style="color:red;">审核失败</span><?php endif; ?>
                        </td>
                        <td><a href="javascript:void(0);" onClick="check_job(this,'<?php echo ($userres["id"]); ?>','<?php echo ($userres["job_status"]); ?>');" style="color: #0000ff;">核查</a></td>
                    </tr>
                </table>
        </div>
    </section>
            ...
            <table border="1" cellspacing="0" class=hk_detail >
                ...
                <?php if(is_array($linkres)): $i = 0; $__LIST__ = $linkres;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
	                    ...
	                    <td style="color: #ff0000;">
	                    	<?php if($v["is_result"] == '1' ): ?><span style="color:green;">审核成功</span>
            ...
            <table id="datalist2" class="easyui-datagrid" title="" style="width:1200px;height:250px"
				...
				<thead>
					<tr>
						...
						<th field="content" align="left" width="20%">本期应还款</th>
						<th field="real_money" align="center" width="15%">汇款金额</th>
						<th field="corpus" align="center" width="15%">借款金额</th>
						<th field="installments" align="center" width="15%">借款期限</th>
						<th field="repay_time" align="center" width="25%">应还款时间</th>
						<th field="real_repay_time" align="center" width="25%">实际还款时间</th>
						<th field="realname" align="center" width="25%">审核员</th>
						<th field="isorno" align="center" width="25%">是否逾期</th>
						<th field="status" align="center" width="25%">状态</th>
					</tr>
				</thead>
			</table>
        </div>
...
<!--------电话历史记录弹出框开始--------- -->
	...
	<div id="dlg_img" class="easyui-dialog" style="width:720px;height:400px;padding:10px 20px" closed="true" buttons="#dlg-buttons">
		<div class="fitem">
			<label>照片记录:</label>
			<p>
			<?php if(is_array($cardhistory)): $i = 0; $__LIST__ = $cardhistory;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; echo ($v); endforeach; endif; else: echo "" ;endif; ?>
			</p>
		</div>
	</div>
	...
	<div id="dlg_phone" class="easyui-dialog" style="width:500px;height:400px;padding:10px 20px" closed="true" buttons="#dlg_phone-buttons">
		<form id="ff" method="post" style="margin-top:40px;" novalidate>
			...
			<input type="hidden" id="link_id" name="link_id" value="<?php echo ($userres["id"]); ?>"/>
		</form>
	</div>
	...
	<!--开始收款-->
	<div id="shoukuan_dialog" class="easyui-dialog" style="width:800px;height:500px;padding:10px 20px" closed="true" buttons="#shoukuan_buttons">
		<form id="ff_shoukuan" method="post" style="margin-top:40px;" novalidate>
			<div class="fitem">
				<label>还款账号：</label>
				<input type="text" id="account" required="true" name="account" />
			</div>
			<div class="fitem">
				<label>账号名：</label>
				<input type="text" id="name"  required="true"name="name" />
			</div>
			<div class="fitem">
				<label>还款银行：</label>
				<input type="text" id="bank"  required="true" name="bank" />
			</div>
			<div class="fitem">
				<label>还款流水号：</label>
				<input type="text"  required="true" name="repay_water_nums" />
			</div>
			<div class="fitem">
				<label>还款金额：</label>
			...
			<div class="fitem">
				...
				<select id="status" name="status">
					<option value="1">已收到款项</option>
					<option value="0">未收到款项</option>
				</select>
			</div>
		</form>
		<div id="shoukuan_buttons">
			<a href="#" id="shoukuan_btn" class="easyui-linkbutton"  iconCls="icon-edit" onClick="submitFormshoukuan()">保存</a>
			<a href="#" class="easyui-linkbutton" iconCls="icon-cancel" onClick="clearFormshoukuan()">取消</a>
		</div>
	</div>
<script type="text/javascript">
	...
	function clearFormshoukuan(){
		$('#shoukuan_dialog').dialog('close');
	}
	//工作信息审核
	...
	//收款的表单的提交。
	function submitFormshoukuan(id){
		url='__URL__/shoukuan/repayid/<?php echo ($_GET['id']); ?>';
		$('#ff_shoukuan').form('submit',{
			url: url,
			onSubmit: function(){
				return $(this).form('validate');
			},
			success: function(result){
				var result = eval('('+result+')');
				if (result.error==0){
					$('#shoukuan_dialog').dialog('close');		// close the dialog
					$.messager.show({
						title: '消息',
						msg: result.mesg
					});
	...
	function show_status(val){
		...
		function shenhe(id){
			$.post('__APP__/Annex/pass',{id:id},function(data){
	...
	//身份证图片
	function fun(){
		var tmp = '<img src="<?php echo ($result["card_front"]); ?>">';
		$("#big").html(tmp);
	...
	function check(){
		var tmp = '<img src="<?php echo ($result["card_back"]); ?>">';
		$("#big").html(tmp);
		var tmps = '<img src="<?php echo ($result["card_back"]); ?>"style="max-width:100%;height:auto;">';
		$("#show").html(tmps);
	}
	function hand(){
		var tmp = '<img src="<?php echo ($result["card_hand"]); ?>" style="max-width:100%;height:auto;">';
		$("#big").html(tmp);
	...
	function show_address(val,row){
			return '<a href="__APP__/Address/index/id/'+row.id+'" class="easyui-linkbutton" iconCls="icon-edit" plain="true"  target="iframe_a">查看</a>'; 
	}
	...
	//收款
	function shenhe_shoukuan(){
	
	 $('#shoukuan_dialog').dialog('open').dialog('setTitle','收款记录');
